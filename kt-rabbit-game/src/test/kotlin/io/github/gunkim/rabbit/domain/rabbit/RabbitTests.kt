...
class RabbitTests : StringSpec({
    "왼쪽으로 움직일 수 있다." {
        ...
        rabbit.move(createPositionPolicy) { Direction.LEFT } shouldBe Rabbit(rabbitId, Position(4))
    }
    "오른쪽으로 움직일 수 있다." {
        val rabbit = Rabbit(rabbitId, Position(5))

        rabbit.move(createPositionPolicy) { Direction.RIGHT } shouldBe Rabbit(rabbitId, Position(6))
    }
    "움직여야 할 위치가 범위를 벗어날 경우 움직이지 않는다." {
        val rabbit = Rabbit(rabbitId, Position(0))

        rabbit.move(createPositionPolicy) { Direction.LEFT } shouldBe Rabbit(rabbitId, Position(0))
    }
})
